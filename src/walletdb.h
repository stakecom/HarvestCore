...
// Copyright (c) 2016-2018 The Harvest developers
...
public:
    ...
    bool WriteZHCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZHCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZHCSeed_deprecated(uint256& seed);
    bool EraseZHCSeed();
    bool EraseZHCSeed_deprecated();

    bool WriteZHCCount(const uint32_t& nCount);
    bool ReadZHCCount(uint32_t& nCount);
