...
CzHCWallet::CzHCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zhc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZHCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZHCSeed_deprecated()) {
                    LogPrintf("%s: Updated zHC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zhc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zhc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzHCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZHCCount(nCountLastUsed);
    else if (!walletdb.ReadZHCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzHCWallet::Lock()
{
    seedMaster = 0;
}

void CzHCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzHCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZHC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzHCWallet::LoadMintPoolFromDB()
{
...
void CzHCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzHCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzHCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zhcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzHCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZHC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zhcTracker which also adds to database
    pwalletMain->zhcTracker->Add(dMint, true);
...
void CzHCWallet::SeedToZHC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzHCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzHCWallet::GenerateDeterministicZHC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzHCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzHCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
