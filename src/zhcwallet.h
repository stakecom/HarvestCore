...
#ifndef Harvest_ZHCWALLET_H
#define Harvest_ZHCWALLET_H
...
public:
    CzHCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZHC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZHC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Harvest_ZHCWALLET_H
