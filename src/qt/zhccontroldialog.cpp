...
std::set<std::string> ZHcControlDialog::setSelectedMints;
std::set<CMintMeta> ZHcControlDialog::setMints;

ZHcControlDialog::ZHcControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZHcControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZHcControlDialog::~ZHcControlDialog()
{
    delete ui;
}

void ZHcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHcControlDialog::updateList()
{
...
void ZHcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZHc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHcControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZHcControlDialog::GetSelectedMints()
{
...
void ZHcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
